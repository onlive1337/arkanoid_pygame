    ...
    for brick in bricks[:]:
        ...
        if brick.hp <= 0:
            if random.random() < cfg.BONUS_PROBABILITY:
                ...
                powerups.append(PowerUp(brick.rect.centerx, brick.rect.centery, bonus_type))
            bricks.remove(brick)
            scored += 10
    return scored
...
def main():
    ...
    while running:
        ...
        for ball in balls:
            state["score"] += _handle_ball_vs_bricks(ball, bricks, powerups)
            if ball.rect.colliderect(paddle.rect) and ball.vy > 0:
                _handle_ball_vs_paddle(ball, paddle)
            ball.update()

        # Drop balls that fall past the paddle
        ...
        # PowerUps: fall, get caught by paddle, or fall off screen
        for bonus in powerups[:]:
            bonus.update()
